 ...
 */
package com.ja.ssas.tabular.graph.algo;

import com.ja.ssas.tabular.graph.CustomEdge;
...
{
    private final Graph<TableVertex, CustomEdge> graph;
     ...
     */
    public AllDirectedPaths(Graph<TableVertex, CustomEdge> graph)
    {
     ...
     */
    public List<GraphPath<TableVertex, CustomEdge>> getAllPaths(
        TableVertex sourceVertex, TableVertex targetVertex, boolean simplePathsOnly, Integer maxPathLength)
    {
        return getAllPaths(
     ...
     */
    public List<GraphPath<TableVertex, CustomEdge>> getAllPaths(
        Set<TableVertex> sourceVertices, Set<TableVertex> targetVertices, boolean simplePathsOnly,
        Integer maxPathLength)
    {
        ...
        Map<CustomEdge, Integer> edgeMinDistancesFromTargets =
     ...
     */
    private Map<CustomEdge, Integer> edgeMinDistancesBackwards(Set<TableVertex> targetVertices, Integer maxPathLength)
    {
        /*
         ...
         */
        Map<CustomEdge, Integer> edgeMinDistances = new HashMap<>();
        Map<TableVertex, Integer> vertexMinDistances = new HashMap<>();
        Queue<TableVertex> verticesToProcess = new LinkedList<>();
        ...
        for (TableVertex vertex; (vertex = verticesToProcess.poll()) != null;) {
            ...
            // decorated
            for (CustomEdge edge : graph.incomingEdgesOf(vertex)) {
                ...
                TableVertex edgeSource = graph.getEdgeSource(edge);
     ...
     *         constraints
     */
    private List<GraphPath<TableVertex, CustomEdge>> generatePaths(
        Set<TableVertex> sourceVertices, Set<TableVertex> targetVertices, boolean simplePathsOnly,
        Integer maxPathLength, Map<CustomEdge, Integer> edgeMinDistancesFromTargets)
    {
        /*
         ...
         */
        List<GraphPath<TableVertex, CustomEdge>> completePaths = new ArrayList<>();
        Deque<List<CustomEdge>> incompletePaths = new LinkedList<>();
        ...
        for (TableVertex source : sourceVertices) {
            ...
            for (CustomEdge edge : graph.outgoingEdgesOf(source)) {
                ...
                {
                    List<CustomEdge> path = Collections.singletonList(edge);
        ...
        for (List<CustomEdge> incompletePath; (incompletePath = incompletePaths.poll()) != null;) {
            Integer lengthSoFar = incompletePath.size();
            assert (maxPathLength == null) || (lengthSoFar < maxPathLength);

            CustomEdge leafEdge = incompletePath.get(lengthSoFar - 1);
            TableVertex leafNode = graph.getEdgeTarget(leafEdge);

            Set<TableVertex> pathVertices = new HashSet<>();
            for (CustomEdge pathEdge : incompletePath) {
                pathVertices.add(graph.getEdgeSource(pathEdge));
                pathVertices.add(graph.getEdgeTarget(pathEdge));
            }

            for (CustomEdge outEdge : graph.outgoingEdgesOf(leafNode)) {
                ...
                {
                    List<CustomEdge> newPath = new ArrayList<>(incompletePath);
                    newPath.add(outEdge);
                    ...
                    if (targetVertices.contains(graph.getEdgeTarget(outEdge))) {
                        GraphPath<TableVertex, CustomEdge> completePath = makePath(newPath);
     ...
     */
    private GraphPath<TableVertex, CustomEdge> makePath(List<CustomEdge> edges)
    {
        TableVertex source = graph.getEdgeSource(edges.get(0));
        TableVertex target = graph.getEdgeTarget(edges.get(edges.size() - 1));
